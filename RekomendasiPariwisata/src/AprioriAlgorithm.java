    ...
    private void createNextItemset() {
        ...
        for (int i = 0; i < this.itemSets.size(); i++) {
            for (int j = i + 1; j < this.itemSets.size(); j++) {
                int[] arr1 = this.itemSets.get(i);
                int[] arr2 = this.itemSets.get(j);
    ...
    private void countFrequentItemset() throws Exception {
        ...
        for (int i = 0; i < this.itemSets.size(); i++) {
            ...
            if ((nSameItemset[i] / (double) (this.nDataSurvei)) >= this.minSupport) {
                printKandidat(this.itemSets.get(i), nSameItemset[i]);
                tempKandidat.add(this.itemSets.get(i));
